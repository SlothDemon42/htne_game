...
class Player (object):
    ...
    def update(self, dir):
        ...
        if not self.falling and not self.jumping and (leftHeld or rightHeld):
            self.spriteName="./img/Run"+str(self.spriteNum)+".png"
            self.spriteNum+=1;
            if self.spriteNum>3:
                self.spriteNum=0
        else: self.spriteName="./img/Stand.png"
...
class Obstacle (object):
    ...
    def load(self):
        ...
        #if self.spriteNum==15 or (self.spriteNum>32 and self.spriteNum<38) 
...
image=pygame.image.load("./HTNE_image.png")
image=pygame.transform.scale(image,(screenWidth,screenHeight))
imageMask=pygame.image.load("./HTNE_stage.png")
